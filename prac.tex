...
\begin{document}
	...
	\subsection*{ACRONYM}
	\include{acronym}
	\addcontentsline{toc}{subsection}{List of Acronyms}
	\newpage
	\pagenumbering{arabic}
\section{INTRODUCTION}
\subsection{Background}
According to the Information and Communication Technology \ac{ICT} Master Plan 2013-2017, see[1], the long term
...
has considered the use of \ac{ICT} as essential to achieve the goals of education. And the prime components of it are:
\begin{enumerate}
\item \ac{ICT} infrastructure including internet connectivity
\item Human resources well trained in the use of the \ac{ICT} infrastructure
\item Availability of the Teaching / Learning Materials in the \ac{ICT} infrastruc-
ture
\item System Enhancement procedures
\end{enumerate}

Village Tech Solutions \ac{VTS} which was established in the year 1996 by David and Hyadi Sowerwine, with the mission to provide safe, efficient and inexpensive energy systems for the people of rural Nepal. \ac{VTS} had been endeavoring to bring audio-visual system to the classrooms of Nepal to enhance their current education since the year 2008. By introducing such multimedia systems into the classroom, their aim is to make standard learning materials available to all Nepalese students irrespective of their socio-economic statuses. The information that can be gained from the introduction of such multi-media to rural villages can help not only the
...
And now with the release of \ac{ICT} master plan for education, the government
of Nepal has been cooperating with \ac{VTS} regarding the same. Also, \ac{VTS}
...
The existing wand system of Looma is basically an IR based handheld device that acts a mouse. It emits \ac{IR} light, and also called a light pen. The major components of it are that it has an \ac{IR} \ac{LED}, a button and and on/off switch and uses 555 timer to turn the IR light source on/off. The led is blinked at a pre-determined frequency so that the blink is be analysed as a click by its camera processor. The current design uses Nintendo Wii \ac{IR} camera, which is scavenged from Wii motes in order to detect the \ac{IR} light on the projected screen. Some of the system's issues which can not be ignored are:

\begin{itemize}
\item \textbf {Drawing in the Whiteboard:} While drawing in the LOOMA software, the tracking of the \ac{IR} led is very unreliable and the drawings are not made as expected.
\item \textbf{Use of Pre-built Multi-Object-Tracking \ac{MOT} processor:} There is the use of inbuilt \ac{MOT} processor in the camera chip, which does part of the image processing and tracking and sending coordinates. In the case of errors, its response can not be modified to remove errors which makes it inefficient
\item \textbf{Errors not detectable:} Since the Wiimote used \ac{IR} to move the mouse, the location of the \ac{IR} on the screen is not visible with the naked eye thus making errors not detectable 
\item \textbf{Unavailability in the market:} Wiimote \ac{IR} camera chip being used in the system can not be readily bought in the open market which makes it difficult to mass produce the system
...
In short, hacked Wii Remotes along with the Field Programmable Gate Array \ac{FPGA} board is adding up to the cost significantly. In addition, the system is not very stable and is inefficient to use as points aforementioned, thereby leading to the research on alternative vision-based \ac{HCI}s and our project is an effort to get
a prototype of one such alternative.

\subsection{Objectives}
The basic objectives of our project is make prototype for an alternative type of vision-based \ac{HCI}s and in the modification of Project Looma in aspects listed below:
\begin{itemize}
	\item To use laser pointer instead of \ac{IR} (Infra-red) based interaction
...
\item Hacked Wii \ac{IR} Camera and Wii wands
\end{itemize}
...
\begin{itemize}
\item Wand shown in the figure is a \ac{3D} printed \ac{IR} light source
\item The wand design uses 555 timer to turn the \ac{IR} light source on/off at a pre-determined frequency such that the blink can be interpreted as a click on the device
\item Current design uses Nintendo Wii \ac{IR} camera, which are scavenged from Wii motes
\end{itemize}
...
The existing system uses Wii Remote to interact in the audio-visual device. This system uses an Wii \ac{IR} camera which picks up the infrared light source, and tracks it using its \ac{MOT} (Multi-Object Tracking) processor present in the camera chip as shown in the figure. Wii \ac{IR} Camera gives out a pre-set \ac{I2C} address so the interface board needs an \ac{I2C} multiplexer. To convert the \ac{I2C} output to serial, and to demultiplex the signals, the system has been using an \ac{FPGA} board. 

\newpage
\section{THEORITICAL BACKGROUND}
\subsection{Basic Overview}
\subsubsection{Human-computer interaction (\ac{HCI})} 
...
It is the first stage of any vision system as all the other stages of image processing begin only after the acquiring of the digital image. The digital image is captured with the help of image sensor. An image sensor is a device that converts an optical image into an electronic signal such as, digital cameras, camera modules and other imaging devices. Here, Raspberry \ac{CSI} camera is used for image acquisition.
...
\paragraph {Proteus \ac{VSM}}
The Proteus \ac{VSM} contains mixed mode SPICE circuit simulation, animated components and microcontroller models to facilitate co-simulation of complete microcontroller based design. It is possible to develop and test such designs before a physical prototype is constructed. This is possible because interaction with the design is possible using circuit indicators like \ac{LED}, display panels, actuators etc. It also provides extensive debugging facility by employing breakpoints, single stepping and variable display of both assembly code and high level language source code.
...
The software of Raspberry Pi utilizes \ac{Rpi} \ac{GPU} (Graphical Processor Unit) when using Raspi Camera Module. So for example encoding \emph {h.264}, video has low impact on the \ac{CPU} (Central Processor Unit) usage. Also, it has an excellent resolution of 5 Megapixels, which is higher than most USB webcams. It also has an excellent daytime image quality. If we use USB Webcam, it will have a very slow frame rate video and the \ac{CPU} usage will be quite high. RPi does not have enough \ac{CPU} horsepower to do higher frame rates, resolution and advanced video compression. 
...
\hline
	1 & \ac{FPGA} Board and Wiring & \$117 & Raspberry Pi & \$64.90\\
\hline
	2 & Nintendo Wii Remote & \$68.89 & Raspberry Pi Camera Module & \$29.95 \\
\hline
	3 & 3D Printed \ac{IR} Wands & \$20 & Laser Pointer & \$0.9\\
\hline
	4 &  &  & Atmega8L & \$1.87\\
\hline
...
\item Human–computer interaction \ac{HCI}
...
\item Structure from motion \ac{SFM}
\item Motion tracking
\item Augmented reality
\end{itemize}

\ac{OpenCV} has more than 47 thousand people of user community and estimated number of downloads exceeding 7 million. The library is used extensively in companies, research groups and by governmental bodies.

It has C++, C, Java and MATLAB interfaces and supports Windows, Linux, Android and Mac OS. \ac{OpenCV} leans mostly towards real-time vision applications and takes advantage of MMX and SSE instructions when available. A full-featured CUDA and OpenCL interfaces are being actively developed right now. There are over 500 algorithms and about 10 times as many functions that compose or support those algorithms. \ac{OpenCV} is written natively in C++ and has a templated interface that works seamlessly with \ac{STL} containers.
\end{document}