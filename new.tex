...
The image coordinates captured by the Raspberry pi CSI camera is mapped to the system coordinates such that the location of the laser point on the image corresponds to the intended system location. The corresponding scaling matrix used for this mapping causes the movement of the laser on the projector screen at each instant to resemble the mouse movement on the system producing the mouse click and drag on the system as desired based on the number of transitions of the laser on and off condition.
...
The projected screen before programmatic exposure correction can be viewed in Appendix H. After the exposure exposure correction is done, only the laser pointer is the visible white dot in the thresholded image. The exposure corrected image can be viewed in Appendix I.
